  ...
  const comedianElements = bookingComedians.map(({ comedian, performance }) => {
    ...
    comedianElement.innerHTML = `
      ...
      <button class="booking__hall" type="button" data-booking = "${
        clientData.fullName
      } ${clientData.ticketNumber} ${comedian.comedian} ${performance.time} ${
      performance.hall
    }">${[performance.hall]}</button>
    `;
    return comedianElements;
  });
